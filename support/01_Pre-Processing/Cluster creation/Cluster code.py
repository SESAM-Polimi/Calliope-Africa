...
#%%
# Create a dictionary to store MSR IDs for each cluster
clustered_zones = {}
...
for ctry in nclust_all:
    ...
    if not raw_ctry.empty:
        ...
        #K-Means Clustering
        model = TimeSeriesKMeans(n_clusters=nclust, metric=metric, max_iter=it, random_state=42, verbose=1).fit(raw_trunc.T)
        labels = model.labels_
        
        # Save MSR IDs for each cluster
        for label in set(labels):
           if label not in clustered_zones:
               clustered_zones[label] = []
           clustered_zones[label] += raw_trunc.columns[labels == label].tolist()
               ...
               for i in range(len(labels)):
                   if (labels[i] == label):
                       for row_i in range(4):
                           axs[row_i].plot(raw_trunc.iloc[plot_hours[row_i]:plot_hours[row_i]+72,i], c="gray", alpha=0.4)
                           cluster.append(raw_trunc.iloc[:,i])
               if len(cluster) > 0:
                   for row_i in range(4):
                       axs[row_i].plot(range(1+plot_hours[row_i],1+plot_hours[row_i]+72),all_table_profiles.loc[label][plot_hours[row_i]:plot_hours[row_i]+72], c="red")
           #
...
#%%
# Now save the clustered zones' MSR IDs into the new Excel file as requested
# =============================================================================
# output_data = []
# for cluster_id, msr_ids in clustered_zones.items():
#     #First row: Cluster ID and number of zones (length of msr_ids)
#     output_row = [cluster_id + 1, len(msr_ids)] + msr_ids  # Adjusting cluster_id to start at 1 instead of 0 
...
# 
# # Add column names
# df_output.columns = ['Cluster ID', 'Zones count'] + ['MSR'] * (df_output.shape[1] - 2)
# 
# # Save to Excel
# output_file = 'Zone_identification.xlsx'
