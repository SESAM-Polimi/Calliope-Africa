...
average_supply= 31.426000000029504 / 0.8731605630976736
...
main_folder_Base = f'Results/RO/50%Solar_50%Wind'
...
for subfolder in subfolders:
    ...
    #calculate solar capacity
    Solar_Capacity=Capacity.loc[(Capacity['techs']=='PV_added') & (Capacity['locs']=='NGA_E') ]
    
    Solar_Capacity=Solar_Capacity.loc[:,'energy_cap'].sum()*1e-6
    
    Wind_Capacity=Capacity.loc[(Capacity['techs']=='Wind_added')  & (Capacity['locs']=='NGA_E') ]
    ...
    Solar_Energy=Energy.loc[(Energy['techs']=='PV_added') & (Energy['locs']=='NGA_E')]

    Solar_Energy=Solar_Energy.loc[:,'carrier_prod'].sum()*1e-9

    
    Wind_Energy=Energy.loc[(Energy['techs']=='Wind_added')  & (Energy['locs']=='NGA_E')]
...
main_folder_Base = f'Results/TE/50%Solar_50%Wind'
...
main_folder_PH = f'Results/PHES/50%Solar_50%Wind'
...
main_folder_PH = f'Results/PHES_TE/50%Solar_50%Wind'
...
main_folder_PH = 'Results/Minimized_PHES/PHES/50%Solar_50%Wind_PHES_min'
...
main_folder_PH = 'Results/Minimized_PHES/PHES_TE/50%Solar_50%Wind_PHES_min'
...
Solar_cap_Base = [x for x in Solar_cap_Base if x <= 25 and x>=5]
Solar_cap_Base_exp = [x for x in Solar_cap_Base_exp if x <= 25 and x>=5]
Solar_cap_PH = [x for x in Solar_cap_PH if x <= 25 and x>=5]
Solar_cap_PH_exp = [x for x in Solar_cap_PH_exp if x <= 25 and x>=5]
Solar_cap_Min = [x for x in Solar_cap_Min if x <= 25 and x>=5]
Solar_cap_Min_exp = [x for x in Solar_cap_Min_exp if x <= 25 and x>=5]
...
marker_x_Solar = [2.5, 5.0, 10.0, 15.0, 20.0, 25.0]
...
Wind_cap_Base = [x for x in Wind_cap_Base if x <= 25 and x>=5]
Wind_cap_Base_exp = [x for x in Wind_cap_Base_exp if x <= 25 and x>=5]
Wind_cap_PH = [x for x in Wind_cap_PH if x <= 25 and x>=5]
Wind_cap_PH_exp = [x for x in Wind_cap_PH_exp if x <= 25 and x>=5]
Wind_cap_Min = [x for x in Wind_cap_Min if x <= 25 and x>=5]
Wind_cap_Min_exp = [x for x in Wind_cap_Min_exp if x <= 25 and x>=5]
...
marker_x_Wind = [5.0, 10.0, 15.0, 20.0, 25.0]
...
#%% resize SW
RE_cap_Base = [x for x in RE_cap_Base if x <= 25 and x>=5]
RE_cap_Base_exp = [x for x in RE_cap_Base_exp if x <= 25 and x>=5]
RE_cap_PH = [x for x in RE_cap_PH if x <= 25 and x>=5]
RE_cap_PH_exp = [x for x in RE_cap_PH_exp if x <= 25 and x>=5]
RE_cap_PH_Min = [x for x in RE_cap_PH_Min if x <= 25 and x>=5]
RE_cap_PH_Min_exp = [x for x in RE_cap_PH_Min_exp if x <= 25 and x>=5]
...
marker_x_SW = [5.0, 10.0, 15.0, 20.0, 25.0]
...
ax7.plot(x1, y1,color='#64b551', linewidth=8, label='RO', marker='o') #verde
...
ax7.scatter(marker_x_Solar, marker_y_Solar_Base, color='#64b551', s=2000)
...
ax15.set_title('Optimal mix: 50% Solar, 50% Wind',  fontweight='bold',fontsize=60, pad=50)
...
ax3.bar(marker_x_Solar, Solar_PHES_size, color='#5c7ce6',width=2.25, label='PHES size')
...
ax4.yaxis.set_major_locator(ticker.MultipleLocator(base=3))
...
ax5.bar(marker_x_Wind, Wind_PHES_size, color='#5c7ce6',width=2.25, label='PHES size')
...
ax6.yaxis.set_major_locator(ticker.MultipleLocator(base=3))
...
ax16.bar(marker_x_SW, SW_PHES_size, color='#5c7ce6',width=2.25, label='PHES size')
...
ax17.yaxis.set_major_locator(ticker.MultipleLocator(base=3))
...
ax9.bar(marker_x_Solar, Solar_exp_PHES_size, color='#5c7ce6',width=2.25, label='PHES size')
...
ax11.bar(marker_x_Wind, Wind_exp_PHES_size, color='#5c7ce6',width=2.25, label='PHES size')
...
ax18.bar(marker_x_SW, SW_exp_PHES_size, color='#5c7ce6',width=2.25, label='PHES size')
...
ax19.yaxis.set_major_locator(ticker.MultipleLocator(base=5))
