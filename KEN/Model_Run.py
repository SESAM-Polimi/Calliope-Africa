...
demand= -pd.read_csv('Timeseries/Demand.csv').loc[:,('KEN_WSTR', 'KEN_MTKR', 'KEN_CSTR', 'KEN_NBOR')].sum().sum()


#%% INPUT DATA

#cumulative capacity of additional wind and solar technologie (in MW)
capacity=list(range(250,3751, 500)) + [3500, 4000, 6000]
...
integration_strategies = ['RO', 'TE', 'PHES', 'PHES_TE']        #
                                                                #                                      
#################################################################
integration_strategies = ['PHES', None]
...
strategy_definition = {
    ...
    'PHES': {'model_file': 'model_transmission.yaml', 'PHES_cap': 33333333, 'PHES_en': 200000000},       #PHES capacity and energy in kW
    'PHES_TE': {'model_file': 'model_transmission_exp.yaml', 'PHES_cap': 33333333, 'PHES_en':200000000} #PHES capacity and energy in kW
}
...
for strategy in integration_strategies:
    ...
    for cap in capacity:
        for sh in share:
            ...
            Location_Constraints['locations']['KEN_WSTR']['techs']['PV_added']['constraints']['energy_cap_equals']=Solar_cap
            Location_Constraints['locations']['KEN_MTKR']['techs']['Wind_added']['constraints']['energy_cap_equals']=Wind_cap
